...
helm install -f influxdb-values.yaml --set setDefaultUser.user.password=${INFLUXDB_ADMIN_PASSWORD} influxdb influxdata/influxdb \
...
kubectl exec ${INFLUXPOD} -n influxdb -- influx -username admin -password ${INFLUXDB_ADMIN_PASSWORD} -execute "$CMD"
